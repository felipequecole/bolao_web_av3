...
class VerPalpites extends React.Component {
    ...
    async componentDidMount() {
        const responseSelecoes = await
            fetch('http://localhost:8080/BolaoDaCopaV2_RS/webresources/palpite/selecoes');
        ...
        if (contentTypeSelecoes && contentTypeSelecoes.includes("application/json")) {
            ...
            const selecoesOptions = selecoesJson.map((selecao) => {
                return {
                    label: selecao, value:
        ...
        const responsePalpites = await
            fetch('http://localhost:8080/BolaoDaCopaV2_RS/webresources/palpite');
    ...
    render() {
        var header = <div style={{ 'textAlign': 'left' }}>
            ...
            <InputText type="search" onInput={(e) => this.setState({
                globalFilter: e.target.value

            })} placeholder="Pesquisar em todos os campos" size="50" />
        </div>;
        let campeaoFilter = <Dropdown style={{ width: '100%' }} className="ui-column-filter"
            value={this.state.filters.campeao ? this.state.filters.campeao.value : null}
            options={this.state.selecoes} onChange={(e) => this.onCampeaoChange(e)} />
            ...
            <div>
                ...
                <div className="content-section implementation">
                    <DataTable value={this.state.palpites} paginator={true} rows={10}
                        ...
                        <Column field="palpiteiro.nome" header="Palpiteiro" filter={true}

                            filterMatchMode='contains' />

                        <Column field="campeao" header="Campeão" filter={true}
